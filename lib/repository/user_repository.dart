...
class UserRepositoryImpl implements UserRepository {
  ...
  final Map<String, String> _userHeader = {'content-type': 'application/json', "Access-Control_Allow_Origin": "*"};
  ...
  Future<void> signInUser(String bodyInput) async {
    ...
    Map<String, dynamic>responseData = await networkHelper.postData(urlInput: 'auth/sign_in', headerInput: _userHeader, bodyInput: bodyInput);
    await prefs.setString('access-token', responseData['headers']['access-token']);
  ...
  Future<void> signUpUser(String bodyInput) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    NetworkHelper networkHelper = NetworkHelper();
    Map<String, dynamic>responseData = await networkHelper.postData(urlInput: 'auth', headerInput: _userHeader, bodyInput: bodyInput);
    await prefs.setString('access-token', responseData['headers']['access-token']);
    await prefs.setString('uid', responseData['headers']['uid']);
    await prefs.setString('client', responseData['headers']['client']);
    Map<String, dynamic> responseBody = jsonDecode(responseData['body']);
  ...
  Future<void> signOutUser() async {
    final prefs = await SharedPreferences.getInstance();
    Map<String, dynamic> _userInfo = {'access-token': prefs.getString('access-token')};
    _userInfo['uid'] = prefs.getString('uid');
    _userInfo['client'] = prefs.getString('client');
    String bodyInput = jsonEncode(_userInfo);
    NetworkHelper networkHelper = NetworkHelper();
    await networkHelper.deleteData(urlInput: 'auth/sign_out', headerInput: _userHeader, bodyInput: bodyInput);
