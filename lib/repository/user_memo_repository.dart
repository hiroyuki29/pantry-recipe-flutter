...
final userMemoRepository =
Provider.autoDispose<UserMemoRepository>((ref) => UserMemoRepositoryImpl(ref.read));
...
class UserMemoRepositoryImpl implements UserMemoRepository {
  ...
  Future<List<Memo>> getUserMemoList() async {
    ...
    var responseData =
    await networkHelper.getData(urlInput: 'memo_users', headerInput: _userInfo);
  ...
  Future<bool> saveUserMemo(Map<String, dynamic> bodyInputMap) async {
    ...
    Map<String, dynamic> response = await networkHelper.postData(
        urlInput: 'memos', headerInput: _userHeader, bodyInput: jsonEncode(bodyInputMap));
    ...
    if (responseBody['status'] == 'SUCCESS'){
      ...
      await networkHelper.postData(
          urlInput: 'memo_users', headerInput: _userHeader, bodyInput: jsonEncode(bodyInputMap));
      return true;
    } else{
      return false;
    }
  ...
  Future<bool> searchMemo(Map<String, dynamic> bodyInputMap) async {
    NetworkHelper networkHelper = NetworkHelper();
    Map<String, dynamic> response = await networkHelper.postData(
        urlInput: 'memos/search', headerInput: _userHeader, bodyInput: jsonEncode(bodyInputMap));
    Map<String, dynamic> responseBody = jsonDecode(response['body']);
    if (responseBody['status'] == 'SUCCESS'){
      dynamic responseBody = response['body'];
      dynamic responseData = jsonDecode(responseBody)['data'];
  ...
  Future<void> deleteUserMemo(Memo memo) async {
    ...
    await networkHelper.deleteData(
        urlInput: 'memos/$id', headerInput: _userHeader, bodyInput: jsonEncode(_userInfo));
  }
}
